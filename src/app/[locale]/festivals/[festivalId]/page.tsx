...
export default async function FestivalPage({ params }: { params: Promise<{ festivalId: string }> }) {
  ...
  const tf = await getTranslations('festivals')
  ...
  return (
    <div className="min-h-screen max-w-2xl mx-auto p-4 pb-24">
      ...
      <div className="mb-6">
        <p className="text-xs font-black uppercase tracking-widest text-[#999] mb-3">{t('create_title')}</p>
        ...
        <div className="space-y-3">
          {(!myGroups || myGroups.length === 0) && (
            <div className="text-center py-10 text-[#999] bg-white border border-dashed border-[#E0D9CC] rounded-2xl">
              <p className="font-medium">{t('no_groups')}</p>
              <p className="text-sm mt-1">{t('no_groups_sub')}</p>
            </div>
          )}
          {myGroups?.map((g: any) => (
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-[#999] mt-0.5">
                      {g.group_members?.[0]?.count ?? 0} {t('members')}
                    </p>
                  </div>
