...
export default function JoinGroupButton({ festivalId, userId, username }: {
  ...
  async function join() {
    ...
    const { data: group } = await supabase
      ...
      .single()
    if (!group) { toast(tc('error'), 'error'); setLoading(false); return }
    const { error: memberError } = await supabase
      .from('group_members')
      .insert({ group_id: group.id, user_id: userId, display_name: username })
    if (memberError && !memberError.message.includes('duplicate')) {
      setError(tc('error')); setLoading(false); return
    }
    toast(t('join_title'), 'success')
      ...
      {open && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm border border-[#E0D9CC] shadow-lg">
            <h2 className="font-black text-xl uppercase tracking-tight mb-4">{t('join_title')}</h2>
            <input
              value={code}
